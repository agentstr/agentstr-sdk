...
def deploy(
    ...
    if config.get("agent_vault_key_manager"):
        ...
        if key_manager.lower() == 'secure':
            val = 'secure'
        elif key_manager.lower() == 'none':
            val = 'none'
        else:
            raise click.ClickException(f"Invalid agent_vault_key_manager: {key_manager}. Must be 'secure' or 'none'.")
        ...
        if val == 'secure':
            if provider.name == 'gcp':
                raise click.ClickException("GCP provider does not support secure agent vault.")
